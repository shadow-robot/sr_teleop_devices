...
class CybergloveCalibrer:
    ...
    def get_calibration_steps(self):
        ...
        self.calibration_steps.append(CalibrationStep(step_name="Joints 0s, 3s and thumb abduction (THJ4)",
                                                      step_description=["Hand flat on a table, fingers joined, thumb " +
                                                                        "opened",
        ...
        self.calibration_steps.append(CalibrationStep(step_name="Joints 4s + TH1, 2, and WR2",
                                                      step_description=["Hand flat fingers joined, thumb completely " +
                                                                        "curled under the palm, wrist 2 bent to " +
                                                                        "the left",
                                                                        "Hand flat fingers apart, thumb completely " +
                                                                        "opened, wrist bent 2 to the right"],
                                                      joints=joints2))
        ...
        self.calibration_steps.append(CalibrationStep(step_name="Joint TH5",
                                                      step_description=["thumb completely under the palm",
                                                                        "thumb completely opened, curled over the " +
                                                                        "palm"],
                                                      joints=joints3))
    ...
    def write_calibration_file(self, filepath):
        ...
        for name in self.joints:
            ...
            # min value
            text.append("<calib raw_value=\""+str(cal.raw_min) + "\" calibrated_value=\"" +
                        str(cal.calibrated_min)+"\"/>")

            # max value
            text.append("<calib raw_value=\""+str(cal.raw_max) + "\" calibrated_value=\"" +
